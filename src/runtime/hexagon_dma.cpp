...
static int halide_hexagon_dma_wrapper (void *user_context, struct halide_buffer_t *src,
    ...
    // TODO: 2D support only, until 3D issues are resolved (deinterleaving of UV with copy_to_*())    
    t_eDmaFmt currentFmt = dev->fmt;
    halide_assert(user_context, dst->dimensions < 3);
    ...
    if ((currentFmt == eDmaFmt_NV12_UV) ||
        ...
        stDmaTransferParm.u16RoiY = (stDmaTransferParm.u16RoiY - src->dim[1].min) * 2;
        debug(user_context) << "u16Roi(X: " << stDmaTransferParm.u16RoiX << " Y: " << stDmaTransferParm.u16RoiY << " W: "
            << stDmaTransferParm.u16RoiW << " H: " << stDmaTransferParm.u16RoiH << ")"
...
WEAK int halide_hexagon_dma_unprepare(void *user_context, struct halide_buffer_t *buf) {
    ...
    int err = nDmaWrapper_FinishFrame(dev->dma_engine); //signal end of transfer
    if (dev->cache_buf) {
        HAP_cache_unlock(dev->cache_buf);//unlock and frees the cache
    }
    dev->cache_buf = 0;
...
WEAK int halide_hexagon_dma_copy_to_device(void *user_context, halide_buffer_t *buf) {
    ...
    // TODO: Implement this with dma_write_data.

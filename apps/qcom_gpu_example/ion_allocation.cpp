...
extern "C" {// If this symbol is defined in the stub library we are going to link
...
void alloc_init() {
  ...
  printf("ion handle = %d \n",ion_fd) ;
  if (ion_fd < 0) {
    __android_log_print(ANDROID_LOG_ERROR, "pranavdb", "open('/dev/ion') failed");
  }
}
void alloc_finalize() {
    close(ion_fd);
...
void* alloc_ion(size_t size, int *fd_val) {
  ...
  const int ion_flags = ion_flag_cached;
    ...
    if (handle < 0) {
        __android_log_print(ANDROID_LOG_ERROR, "pranavdb", "ion_alloc(%d, %lu, %lu, %d, %d) failed",
    ...
    if (buf_fd < 0) {
        __android_log_print(ANDROID_LOG_ERROR, "pranavdb", "ion_map(%d, %d) failed", ion_fd, handle);
    ...
    *fd_val = buf_fd;
    if (buf == MAP_FAILED) {
        __android_log_print(ANDROID_LOG_ERROR, "pranavdb", "mmap(NULL, %lu, PROT_READ | PROT_WRITE, MAP_SHARED, %d, 0) failed",
                            size, buf_fd);
        close(buf_fd);
        ion_free(ion_fd, handle);
        return NULL;
    ...
    if (remote_register_buf) {
        printf("Rgistering the allocated Buf"); 
    ...
    if (!rec) {
        __android_log_print(ANDROID_LOG_ERROR, "pranavdb", "malloc failed");
        munmap(buf, size);
        close(buf_fd);
        ion_free(ion_fd, handle);
        return NULL;
    ...
    __android_log_print(ANDROID_LOG_INFO, "pranavdb", "alloc succeeded");
    return buf;
...
void alloc_ion_free(void *ptr) {
    ...
    pthread_mutex_unlock(&allocations_mutex);
    if (!rec) {
        __android_log_print(ANDROID_LOG_WARN, "pranavdb", "Allocation not found in allocation records");
        return;
    }
    ...
    if (ion_free(ion_fd, rec->handle) < 0) {
        __android_log_print(ANDROID_LOG_WARN, "pranavdb", "ion_free(%d, %d) failed", ion_fd, rec->handle);
    }

...
int main(int argc, char **argv) {
    ...
    uint8_t *memory_to_dma_to = (uint8_t *)malloc(width * height*1.5  );
    ...
    halide_hexagon_dma_prepare_for_copy_to_device(nullptr, input, dma_engine, false, eDmaFmt_NV12);
    ...
    int result = pipeline_write_nv12(input, output_y,output_c);
    if (result != 0) {
    ...
    for (int z = plane_start; z < plane_end; z++) {
        ...
        for (int y = 0; y < height_c; y++) {
            for (int x = 0; x < width; x++) {
                uint8_t correct = memory_to_dma_from[x + y*width + z*width*height] ;
